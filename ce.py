...
class CEDriver(NetworkDriver):
    ...
    def get_facts(self):
        ...
        show_serial_number= self.device.send_command('display esn ')
        ...
        for line in show_ver.splitlines():
            ...
            if 'HUAWEI' in line and 'uptime is' in line:
                search_model = re.search(r"HUAWEI\s+(?P<model>\S+)", line)
            ...
            if 'Software      Version' in line:
                search_ver = re.search(r"Software.*\((?P<os_version>V\S+)\)", line)
                if search_ver is not None:
                    os_version = search_ver.group('os_version')
        ...
        #list serial_number
        if 'ESN ' in show_serial_number:
            serial_number=re.findall(r"ESN of slot.*: (?P<sn>\S+)",show_serial_number)
